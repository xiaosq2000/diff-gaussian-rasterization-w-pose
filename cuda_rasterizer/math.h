...
struct mat33 {
  ...
  __host__ __device__ mat33(const float3& c0, const float3& c1,
                            const float3& c2) {
    cols[0] = c0;
    cols[1] = c1;
    cols[2] = c2;
  ...
  __host__ __device__ static mat33 identity() {
    return mat33(make_float3(1, 0, 0), make_float3(0, 1, 0),
                 make_float3(0, 0, 1));
  }

  __host__ __device__ static mat33 skew_symmetric(const float3& v) {
    return mat33(make_float3(0, v.z, -v.y), make_float3(-v.z, 0, v.x),
  ...
  __host__ __device__ mat33 transpose() const {
    ...
    float3 c1 = cols[1];
    float3 c2 = cols[2];
    return mat33(make_float3(c0.x, c1.x, c2.x), make_float3(c0.y, c1.y, c2.y),
...
struct mat34 {
  ...
  __host__ __device__ mat34(const float3& c0, const float3& c1,
                            const float3& c2, const float3& c3) {
    cols[0] = c0;
    cols[1] = c1;
    cols[2] = c2;
    cols[3] = c3;
...
struct mat44 {
  ...
  __host__ __device__ mat44(const float4& c0, const float4& c1,
...
struct SO3 {
  ...
  __host__ __device__ SO3(const mat33& data) { data_ = data; }
  __host__ __device__ mat33 data() const { return data_; }
  ...
  __host__ __device__ SO3 inverse() const { return SO3(data_.transpose()); }
};
...
struct SE3 {
  ...
  __host__ __device__ SE3(const float* data) {
    ...
    R_data_ = SO3(mat33(make_float3(T.cols[0]), make_float3(T.cols[1]),
                        make_float3(T.cols[2])));
  }

  __host__ __device__ SE3(const mat44& data) {
    t_data_ = make_float3(data.cols[3]);
    R_data_ = SO3(mat33(make_float3(data.cols[0]), make_float3(data.cols[1]),
                        make_float3(data.cols[2])));
  }

  __host__ __device__ SO3 R() const { return R_data_; }

  __host__ __device__ float3 t() const { return t_data_; }

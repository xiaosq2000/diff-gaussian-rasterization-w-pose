...
CudaRasterizer::SemanticGeometryState
CudaRasterizer::SemanticGeometryState::fromChunk(char*& chunk, size_t P) {
  SemanticGeometryState semantic_geom;
  obtain(chunk, semantic_geom.depths, P, 128);
  obtain(chunk, semantic_geom.clamped, P * 3, 128);
  obtain(chunk, semantic_geom.internal_radii, P, 128);
...
int CudaRasterizer::SemanticRasterizer::forward(
  ...
  if (radii == nullptr) {
    radii = semantic_geometry_state.internal_radii;
  }
  ...
  // Dynamically resize image-based auxiliary buffers during training
  size_t img_chunk_size = required<ImageState>(image_width * image_height);
  char* img_chunkptr = image_buffer(img_chunk_size);
  ImageState img_state =
      ImageState::fromChunk(img_chunkptr, image_width * image_height);
  ...
  if (num_rendered > 0)
    identifyTileRanges<<<(num_rendered + 255) / 256, 256>>>(
        num_rendered, binning_state.point_list_keys, img_state.ranges);
  CHECK_CUDA(, debug)

  // Let each tile blend its range of Gaussians independently in parallel
  const float* feature_ptr =
      colors_precomp != nullptr ? colors_precomp : semantic_geometry_state.rgb;
...
void CudaRasterizer::SemanticRasterizer::backward(
  ...
  ImageState image_state =
      ImageState::fromChunk(image_buffer, image_width * image_height);
  ...
  // opacity and RGB of Gaussians from per-pixel loss gradients.
  // If we were given precomputed colors and not SHs, use them.
  const float* color_ptr = (colors_precomp != nullptr)
                               ? colors_precomp
                               : semantic_geometry_state.rgb;
  const float* semantics_ptr = (semantics_precomp != nullptr)
                                   ? semantics_precomp
                                   : semantic_geometry_state.semantics;

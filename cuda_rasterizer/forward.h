...
void semantic_preprocess(
    ...
    float* colors, float* semantics, float4* conic_opacity, const dim3 grid,
...
void render(const dim3 grid, dim3 block, const uint2* ranges,
            ...
            const float* bg_color, float* out_color, const float* depth,
            float* out_depth, float* out_opacity, int* n_touched);

void semantic_render(const dim3 grid, dim3 block, const uint2* ranges,
                     const uint32_t* point_list, int W, int H,
                     const float2* points_xy_image, const float* features,
                     const float* semantic_features,
                     const float4* conic_opacity, float* final_T,
                     uint32_t* n_contrib, const float* bg_color,
                     const float* bg_semantics, float* out_color,
                     float* out_semantics, const float* depth, float* out_depth,

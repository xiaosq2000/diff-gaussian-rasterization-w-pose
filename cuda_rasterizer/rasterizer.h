...
class Rasterizer {
 public:
  ...
  static int forward(std::function<char*(size_t)> geometryBuffer,
                     ...
                     const bool prefiltered, float* out_color, float* out_depth,
                     float* out_opacity, int* radii = nullptr,
                     int* n_touched = nullptr, bool debug = false);

  static void backward(const int P, int D, int M, int R,
                       const float* background, const int width, int height,
                       const float* means3D, const float* shs,
                       ...
                       const float* projmatrix, const float* projmatrix_raw,
                       const float* campos, const float tan_fovx,
                       float tan_fovy, const int* radii, char* geom_buffer,
                       char* binning_buffer, char* image_buffer,
                       const float* dL_dpix, const float* dL_dpix_depth,
                       float* dL_dmean2D, float* dL_dconic, float* dL_dopacity,
                       float* dL_dcolor, float* dL_ddepths, float* dL_dmean3D,
                       float* dL_dcov3D, float* dL_dsh, float* dL_dscale,
                       float* dL_drot, float* dL_dtau, bool debug);
};
class SemanticRasterizer {
 public:
  ...
  static int forward(std::function<char*(size_t)> semantic_geometry_buffer,
                     std::function<char*(size_t)> binning_buffer,
                     std::function<char*(size_t)> image_buffer, const int P,
                     int D, int M, const float* background_color,
                     const float* background_semantics, const int image_width,
                     int image_height, const float* means3D, const float* shs,
                     const float* colors_precomp,
                     const float* semantics_precomp, const float* opacities,
                     const float* scales, const float scale_modifier,
                     const float* rotations, const float* cov3D_precomp,
                     const float* viewmatrix, const float* projmatrix,
                     const float* cam_pos, const float tan_fovx, float tan_fovy,
                     const bool prefiltered, float* out_color,
                     float* out_semantics, float* out_depth, float* out_opacity,
                     int* radii = nullptr, int* n_touched = nullptr,
                     bool debug = false);
  ...
  static void backward(
      ...
      const float* background_semantics, const int image_width,
      int image_height, const float* means3D, const float* shs,
      const float* colors_precomp, const float* semantics_precomp,
      ...
      float* dL_ddepths, float* dL_dmean3D, float* dL_dcov3D, float* dL_dsh,

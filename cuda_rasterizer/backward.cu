...
__global__ void preprocessCUDA(
  ...
  float3 d_proj_dp_C1_d_rho =
      dp_C_d_rho.transpose() * d_proj_dp_C1;  // x.T A = A.T x
...
__global__ void __launch_bounds__(BLOCK_X* BLOCK_Y) semantic_render_cuda(
    const uint2* __restrict__ ranges, const uint32_t* __restrict__ point_list,
    int W, int H, const float* __restrict__ background_color,
    const float* __restrict__ background_semantics,
    const float2* __restrict__ means2D,
    const float4* __restrict__ conic_opacity, const float* __restrict__ colors,
    const float* __restrict__ semantics, const float* __restrict__ depths,
    const float* __restrict__ final_Ts, const uint32_t* __restrict__ n_contrib,
    const float* __restrict__ dL_dpixels,
    const float* __restrict__ dL_dpixels_semantics,
    const float* __restrict__ dL_dpixels_depth, float3* __restrict__ dL_dmean2D,
    float4* __restrict__ dL_dconic2D, float* __restrict__ dL_dopacity,
    float* __restrict__ dL_dcolors, float* __restrict__ dL_dsemantics,
...
void BACKWARD::semantic_render(
    const dim3 grid, const dim3 block, const uint2* ranges,
    const uint32_t* point_list, int W, int H, const float* bg_color,
    const float* bg_semantics, const float2* means2D,
    const float4* conic_opacity, const float* colors, const float* semantics,
    const float* depths, const float* final_Ts, const uint32_t* n_contrib,
    const float* dL_dpixels, const float* dL_dpixels_semantics,
    const float* dL_dpixels_depth, float3* dL_dmean2D, float4* dL_dconic2D,
    float* dL_dopacity, float* dL_dcolors, float* dL_dsemantics,
    float* dL_ddepths) {
